...
export class ProjectGridComponent implements OnInit{
  ...
  exportToExcel(): void {
    ...
    const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.projectSummaryReport); // or projectDetailReport depending on what you want
    const workbook: XLSX.WorkBook = { 
      Sheets: { 'data': worksheet },
      SheetNames: ['data']
    };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    this.saveExcelFile(excelBuffer, 'Project_Summary_Report.xlsx');
  }
  if(this.projectDetailReport.length>0){
    const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.projectDetailReport); // or projectDetailReport depending on what you want
    const workbook: XLSX.WorkBook = { 
      Sheets: { 'data': worksheet },
      SheetNames: ['data']
    };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    this.saveExcelFile(excelBuffer, 'Project_Detail_Report.xlsx');
  }
  }
